...
namespace Unpack
{
	...
	using Executor = std::function<int(const char* command)>;
	inline int DefaultExecutor(const char* command) { return std::system(command); }
	...
	protected:
		...
		virtual std::string_view DecodeExitCode(int ec) const = 0;
		virtual std::string BuildTestCommand() const;
		virtual std::string BuildPassword() const;

		std::string BuildTestCommand2() const;
	...
	private:
		...
		int ExecuteCommand(const char* cmd) const;
	...
	private:
		...
		std::string_view DecodeExitCode(int ec) const override;
	};
